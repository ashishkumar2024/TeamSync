      ...
      {items.length > 0 && (
        <div className="space-y-4">
          {unreadItems.length > 0 && (
            <div>
              <h2 className="text-sm font-medium text-slate-700 mb-2">Unread ({unreadItems.length})</h2>
          ...
          {items.filter((n: { read: boolean }) => n.read).length > 0 && (
            <div>
              <h2 className="text-sm font-medium text-slate-700 mb-2">Read</h2>
              <ul className="space-y-2">
                {items.filter((n: { read: boolean }) => n.read).map((n: {
                  id: string;
                  title: string;
                  body?: string;
                  type: string;
